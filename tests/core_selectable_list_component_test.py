...
@pytest.fixture
def assert_selected_row_is(snapshot, render):
    def assert_selected_row_is(component, position):
        snapshot.assert_match(render(component), f"position-{position}.png")

    return assert_selected_row_is
...
def assert_select_next_row_selects_row(assert_selected_row_is):
    ...
    return assert_select_next_row_selects_row
...
def assert_select_previous_row_selects_row(assert_selected_row_is):
    ...
    return assert_select_previous_row_selects_row


# def test_rows(create_selectable_list, create_numbered_rows, render, snapshot):
#     try:
...
#     snapshot.assert_match(render(create_selectable_list(Rows=create_numbered_rows(1))), "one-row.png")
#
#     # splits the height equally if there are multiple rows
#     snapshot.assert_match(render(create_selectable_list(Rows=create_numbered_rows(4))), "multiple-rows.png")


def test_num_visible_rows(
    create_selectable_list, create_numbered_rows, render, snapshot
):
    # calculates row height based off num_visible_rows
    snapshot.assert_match(
        render(
            create_selectable_list(Rows=create_numbered_rows(1), num_visible_rows=2)
        ),
    ...
    snapshot.assert_match(
        render(
            create_selectable_list(Rows=create_numbered_rows(2), num_visible_rows=2)
        ),
    ...
    snapshot.assert_match(
        render(
            create_selectable_list(Rows=create_numbered_rows(20), num_visible_rows=4)
        ),
    ...
    snapshot.assert_match(
        render(
            create_selectable_list(
                Rows=create_numbered_rows(10000), virtual=True, num_visible_rows=5
            )
        ),
...
def test_lists_that_cant_select_next_or_previous(
    ...
    component = create_selectable_list(Rows=create_numbered_rows(1), num_visible_rows=2)
    component.select_next_row()
    sleep(0.05)
...
def test_select_next_and_previous(
    ...
    # can select next
    assert_select_next_row_selects_row(component, 2)
    assert_select_next_row_selects_row(component, 3)

    # can select previous when in middle of list
    assert_select_previous_row_selects_row(component, 2)

    # can continue selecting next after selecting previous row
    assert_select_next_row_selects_row(component, 3)
    assert_select_next_row_selects_row(component, 4)
...
def test_select_before_render(
    ...
    component = create_selectable_list(Rows=create_numbered_rows(5), num_visible_rows=3)
    component.select_next_row()
    sleep(0.05)
    snapshot.assert_match(
    ...
    # selecting next does not animate before initial render
    component = create_selectable_list(
        Rows=create_numbered_rows(5), num_visible_rows=3, initial_top_row_index=1
    )
    component.select_previous_row()
    sleep(0.05)
    snapshot.assert_match(
...
def test_cleanup(parent, create_numbered_rows, render):
    from pt_miniscreen.core.components import SelectableList

    component = ref(
        parent.create_child(
            SelectableList, Rows=create_numbered_rows(5), num_visible_rows=3
        )
    )
