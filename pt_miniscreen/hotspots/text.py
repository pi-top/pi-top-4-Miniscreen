...
class TextHotspot(Hotspot):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        text = text if not callable(get_text) else get_text()
        font = font if font is not None else get_font(font_size, bold, italics)
        self._get_text = get_text
        self._align_rounding_fn = align_rounding_fn
    ...
    def _calculate_text_x(self, text, font, width):
        ...
        if self.state["align"] == "center":
            return self._align_rounding_fn((width - text_size[0]) / 2)
    ...
    def _calculate_text_y(self, text, font, height):
        ...
        if self.state["vertical_align"] == "center":
            return self._align_rounding_fn((height - text_size[1]) / 2)
