...
class Root(Component):
    def __init__(self, **kwargs):
        super().__init__(
            **kwargs, initial_state={"asteroid_positions": {}, "ship_position": None}
        )
        ...
        self.create_interval(self._move_asteroids, 1 / 24)
    ...
    def _accelerate_ship(self):
        ...
        self._ship_speed = sqrt(pow(clamped_step[0], 2) + pow(clamped_step[1], 2))
        self._previous_ship_angle = self.ship.state["angle"]
    ...
    def render(self, image):
        ship_pos = self.state["ship_position"] or (
            offset_to_center(image.width, self._ship_area_size[0]),
            offset_to_center(image.height, self._ship_area_size[1]),
        )
...
class App(AppBase):
    def __init__(self):
        super().__init__(miniscreen=Miniscreen(), Root=Root)
